...
const messages = [
  {
    ...
    date: '10:30 AM',
    read: false,
    content: 'Hi, just a quick update! Your order #12345 has been shipped and is on its way. You can track your package using the link below...'
  },
  {
    id: 2,
    ...
    date: 'Yesterday',
    read: true,
    content: 'Welcome to the new and improved World Peas dashboard! We have added new features to help you manage your content more efficiently...'
  },
  {
    id: 3,
    sender: 'Jane Smith',
    ...
    date: '3 days ago',
    read: false,
    content: 'Hello, I was wondering about the origin of your organic ginger. Is it locally sourced? Thanks!'
  },
];
          ...
          <div className="w-full lg:w-1/3 border-r border-gray-200">
            ...
            <ul className="overflow-y-auto">
              {messages.map((message) => (
                ...
                >
                  <p className={`font-semibold ${message.read ? 'text-gray-500' : 'text-gray-800'}`}>
                    {message.sender}
                  </p>
                  <p className="text-sm font-medium">{message.subject}</p>
                  <p className="text-xs text-gray-500 truncate">{message.snippet}</p>
                  <span className="absolute top-4 right-4 text-xs text-gray-500">{message.date}</span>
                </li>
              ))}
            </ul>
          </div>
